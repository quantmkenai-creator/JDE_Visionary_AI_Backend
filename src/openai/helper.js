...
- Recent/top: ORDER BY SDDRQJ DESC (requested date); use MAX(SDDRQJ) if GROUP BY
...
"Recent sales": SELECT TOP 20 sod.SDDOCO AS order_number, MAX(sod.SDDRQJ) AS requested_date, SUM(sod.SDUORG) AS total_qty FROM F4211 sod JOIN F4201 sh ON sh.SHDOCO=sod.SDDOCO GROUP BY sod.SDDOCO ORDER BY MAX(sod.SDDRQJ) DESC
